...
export class SeLogerScraperBotBrowser extends BaseScraper {
  private browser: any = null;
  private page: any = null;
  private botbrowserProcess: ChildProcess | null = null;
  private cdpPort = 9222;
  private cookieStoragePath = path.join(process.cwd(), '.scraper-storage', 'seloger-cookies.json');
  private botbrowserPath = process.env.BOTBROWSER_PATH || '/opt/botbrowser/chrome';
  private profilePath = process.env.BOTBROWSER_PROFILE || '/app/bot-profiles/chrome143_win11_x64.enc';
  ...
  private async launchBotBrowser(): Promise<void> {
    this.logger.info('🤖 Launching BotBrowser...');
    ...
    // Add bot profile if exists
    if (fs.existsSync(this.profilePath)) {
      args.push(`--bot-profile=${this.profilePath}`);
      this.logger.info(`📋 Using profile: ${this.profilePath}`);
    } else {
      this.logger.info('📋 No custom profile, using default fingerprint');
    }

    // Add Tor proxy if available
    if (process.env.TOR_PROXY) {
      args.push(`--proxy-server=${process.env.TOR_PROXY}`);
      this.logger.info(`🧅 Using Tor proxy: ${process.env.TOR_PROXY}`);
    }

    // Add user data dir
    const userDataDir = `/tmp/botbrowser-${Date.now()}`;
    args.push(`--user-data-dir=${userDataDir}`);

    this.botbrowserProcess = spawn(this.botbrowserPath, args, {
      stdio: ['ignore', 'pipe', 'pipe'],
    });

    this.botbrowserProcess.stdout?.on('data', (data) => {
      this.logger.info(`BotBrowser: ${data.toString().trim()}`);
    });

    this.botbrowserProcess.stderr?.on('data', (data) => {
      this.logger.warn(`BotBrowser stderr: ${data.toString().trim()}`);
    });

    // Wait for CDP to be ready
    await sleep(5);
    this.logger.info('✅ BotBrowser launched');
  }

  private async initBrowser(): Promise<void> {
    if (this.browser) return;

    this.logger.info('🥷 Initializing BotBrowser connection...');

    // Launch BotBrowser process
    await this.launchBotBrowser();

    // Connect to BotBrowser via CDP
    ...
    this.logger.info('✅ Connected to BotBrowser');
  ...
  async close(): Promise<void> {
    ...
    if (this.botbrowserProcess) {
      this.botbrowserProcess.kill();
      this.botbrowserProcess = null;
      this.logger.info('🔒 BotBrowser closed');
    }
  }
...
export const selogerScraperBotBrowser = new SeLogerScraperBotBrowser();
